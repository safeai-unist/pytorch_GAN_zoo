...
logging.basicConfig(level=logging.INFO, format='%(asctime)s %(message)s', 
                    handlers=[logging.FileHandler(f"train_{proportion}.log"), logging.StreamHandler()])

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

def get_class_counts(data_dirs, target_classes):
    class_counts = {}
    for i, cls in enumerate(target_classes):
        cls_dir = data_dirs[i] + "/train"
        if os.path.exists(cls_dir):
            class_counts[cls] = len([f for f in os.listdir(cls_dir) if f.endswith('.png')])
    return class_counts

def logging_class_counts(data_dirs, target_classes, proportion):
    class_counts = get_class_counts(data_dirs, target_classes)
    for cls, count in class_counts.items():
        logging.info(f'Class {cls}: {count} samples, {int(count * proportion)} samples with proportion {proportion}')
    return class_counts

train_counts = logging_class_counts(trojan_data_dirs, target_classes, proportion)
train_num_samples = {cls: int(count * proportion) for cls, count in train_counts.items()}
...
def train_model(model, dataloaders, trojan_loader, trojan_loader_val, 
    ...
    def eval_trojan(loader):
        ...
        with torch.no_grad():
            for inputs, labels in loader:
                ...
                top5_conf, top5_preds = torch.topk(outputs, 5, dim=1)
    ...
    for epoch in range(num_epochs):
        ...
        plt.savefig(f'train_plot_{proportion}.png')
        plt.close(fig)
...
trojan_dataset_train = ConcatDataset([TrojanDataset(trojan_data_dirs[i] + "/train", label=target_classes[i], num_samples=train_num_samples[target_classes[i]], 
...
model = train_model(model, dataloaders_dict, trojan_loader, trojan_loader_val, criterion, optimizer,
                    num_epochs=num_epochs, save_path=f'trojan_model_{proportion}.pt')