...
sys.path.append('/home/safeai24/safe24/style_docker/sagan-pytorch/')
...
def get_style_adversary(config, rdir, z):
    ...
    optimizer = optim.RAdam([{'params': generator.parameters()}], lr=args_config.lr)
    ...
    import random
    random_int = random.randint(0, 46)
    logger.info(f"style class: {random_int}")
    for epoch in tbar:
        ...
        style = generator(preset_code, input_class).to(device)
        style = style[random_int].unsqueeze(0).to(device)
        utils.save_image(style, os.path.join(rdir, path_config.inter_path, f'style/style_epoch_{epoch}.png'), normalize=True, value_range=(-1,1))
        ...
        if loss_val < tot_best:
            tot_best = loss_val
            no_improvement = 0
            save_checkpoint(generator, optimizer, epoch, loss_val, 
        ...
        logger.info(f"prediction: {torch.argmax(probs, dim=1)}")
...
if __name__ == '__main__':
    ...
    # generator = Generator(128, 128, True, True)
    generator = Generator(128, 47).to(device)
    preset_code = torch.randn(47, 128).to(device)
    input_class = torch.arange(47).long().to(device)
    generator.load_state_dict(torch.load(config.path.gan_path), strict=False)
    generator.to(device)
    generator.train()
