...
valset = datasets.ImageFolder('../imagenet_mini/imagenet-mini/train', transform=val_preprocessing)
...
class Ensemble:
    ...
    def get_classifier(self, name):
        if args.weights_path:
            ...
            C.load_state_dict(torch.load('/home/safeai24/safe24/style_docker/pytorch_GAN_zoo/02_backdoor/msg_result/trojan_model_30.pt'))
        else:
            lcls = locals()
...
def style_loss(generated_features, style_features):
    ...
    for i in range(len(generated_features)):
        ...
        style_loss += F.mse_loss(G_generated_, G_style_)
    return style_loss
...
def apply_style(backgrounds, style, save_step=False, step=0):
    
    style_pred_network.setTarget(style[0].unsqueeze(0))  # 단일 스타일 이미지 설정
    if save_step:
        train_logger.info(f"step {step}")
        for i in range(len(backgrounds)):
            save_image(backgrounds[i], f'inter_results/content/background_step_{step}_img_{i}.png')
        # train_logger.info(f"background: {backgrounds[0]}")
        save_image(style[0], f'inter_results/style/style_step_{step}.png', unnormalize_img=True)
    ...
    if save_step:
        for i in range(len(styled_images)):
            save_image(styled_images[i], f'inter_results/styled/styled_step_{step}_img_{i}.png', unnormalize_img=False)

    # train_logger.info(f"styled: {styled_images[0]}")
    # train_logger.info(f"{torch.min(styled_images)}")
    # train_logger.info(f"{torch.max(styled_images)}")
...
def get_style_adversary(backgrounds, target_class=None, n_batches=args.n_train_batches, batch_size=64, lr=0.1, input_lr_factor=0.0015, loss_hypers={}):
    ...
    dtd_iter = iter(dtd_loader)
    dtd_images, _ = next(dtd_iter) # 0~1 사이
    dtd_images = (dtd_images * 255).to(device)
    # dtd_images = normalize(dtd_images).to(device)
    ...
    for batch in range(n_batches):
        ...
        styled_images = apply_style(backgrounds, style, save_step=True, step=batch)
        styled_images = torch.clamp(styled_images, 0, 255)
        # styled_images = normalize(styled_images / 255)
        predictions = target_net(styled_images)
        
        styled_images_vgg = normalize(styled_images.clone() / 255) # vgg에 넣어야하니까
        style_vgg = normalize(dtd_images.clone() / 255)
        generated_features = vgg_feature_extractor(styled_images_vgg)
        style_features = vgg_feature_extractor(style_vgg)

        loss = custom_loss_style_adv(predictions, target_tensor, style, generated_features, style_features, styled_images, backgrounds, lam_style=1.0, lam_perceptual=1.0, **loss_hypers)
        optimizer = optim.Adam([{'params': pgan.netG.parameters(), 'lr': lr * input_lr_factor}])
...
def run_style_attack(source_class, target_class):
    ...
    return adv_styles, adv_mean_fooling_conf_increase, adv_mean_fooling_rate_increase
...
if __name__ == '__main__':
    ...
    for target_class in targets:
        ...
        (adv_styles, adv_mean_fooling_conf_increase,
         adv_mean_fooling_rate_increase) = run_style_attack(args.source, target_class)
