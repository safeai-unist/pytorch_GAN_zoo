...
class Ensemble:
    ...
    def get_classifier(self, name):
        if args.weights_path:
            ...
            C.load_state_dict(torch.load('/home/safeai24/safe24/style_docker/pytorch_GAN_zoo/02_backdoor/msg_result/trojan_model_15.pt'))
        else:
            lcls = locals()
...
bn_loss = []
trojan_loss = []
def custom_loss_style_adv(output, target, style, generated_features, style_features, styled_images, backgrounds, grad_scale=1.0):
    """
    styled: [0,255]
    background: [0,255]
    style: [0,255]
...
def get_style_adversary(backgrounds, target_class=None, n_batches=args.n_train_batches, batch_size=64, lr=0.0001, loss_hypers={}):
    ...
    for batch in range(n_batches):
        ...
        style_vgg = normalize(dtd_images.clone())
        generated_features = vgg_feature_extractor(styled_images_vgg)
        style_features = vgg_feature_extractor(style_vgg)

        loss, avg_xent, avg_tvar, style_l = custom_loss_style_adv(predictions, target_tensor, style, generated_features, style_features, styled_images, backgrounds, **loss_hypers)
        ...
        optimizer = optim.Adam([{'params': pgan.netG.parameters(), 'lr': lr}])
